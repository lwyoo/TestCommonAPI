...
class HelloWorldStubAdapter
 ...
 public:
    ...
    * Instead, the "fire<broadcastName>Event" methods of the stub should be used.
    */
    virtual void fireMyEventEvent(const int32_t &_myValue) = 0;
...
public:
    ...
    /// Sends a broadcast event for myEvent.
    virtual void fireMyEventEvent(const int32_t &_myValue) {
        auto stubAdapter = CommonAPI::Stub<HelloWorldStubAdapter, HelloWorldStubRemoteEvent>::stubAdapter_.lock();
        if (stubAdapter)
            stubAdapter->fireMyEventEvent(_myValue);
    }
